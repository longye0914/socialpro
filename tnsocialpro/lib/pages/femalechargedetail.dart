...
import 'package:tnsocialpro/widget/card_incomelist.dart';
import 'package:tnsocialpro/widget/card_withdrawlist.dart';
 ...
 * 女 交易明细
 */
final List<Withdrawlist> withdrawLists = new List<Withdrawlist>();
final List<Incomeitemlist> incomeitemLists = new List<Incomeitemlist>();
String tkV;
class FemaleChargeDetailPage extends StatefulWidget {
  String tk;
  FemaleChargeDetailPage(this.tk) {
    tkV = this.tk;
  }

  @override
  _FemaleChargeDetailPageState createState() => _FemaleChargeDetailPageState();
}

class _FemaleChargeDetailPageState extends State<FemaleChargeDetailPage>
...
class _FindingTabViewState extends State<FindingTabView> with AutomaticKeepAliveClientMixin{
  ...
  void loadData(int id) async{
    if (1 == id) {
      // 收入明细
      try {
        var res = await G.req.shop.getIncomeItemlistReq(
            tk: tkV,
            anthorid: userId
        );
        setState(() {
      ...
      // 提现明细
      try {
        var res = await G.req.shop.getWithdrawlistReq(
            tk: tkV,
            userid: userId
        );
        setState(() {
          if (res.data != null) {
            _isLoading = false;
            withdrawLists.clear();
            withdrawLists.addAll(WithdrawlistParent.fromJson(res.data).data);
    ...
    if(_isLoading){
      ...
      return Container(
        ...
        child: EasyRefresh(
            ...
            child:ListView(
              children: <Widget>[
                (1 == id) ? CardIncomelist(articleData: incomeitemLists, tk: tkV) : CardWithdrawlist(articleData: withdrawLists, tk: tkV)
              ],
            ),
            onRefresh: () async {
              _isLoading = true;
