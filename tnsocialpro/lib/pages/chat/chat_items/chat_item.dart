...
import 'package:tnsocialpro/pages/chat/chat_items/chat_file_bubble.dart';
import 'package:tnsocialpro/pages/chat/chat_items/chat_image_bubble.dart';
import 'package:tnsocialpro/pages/chat/chat_items/chat_location_bubble.dart';
import 'package:tnsocialpro/pages/chat/chat_items/chat_text_bubble.dart';
import 'package:tnsocialpro/pages/chat/chat_items/chat_video_bubble.dart';
import 'package:tnsocialpro/pages/chat/chat_items/chat_voice_bubble.dart';
...
class ChatItem extends StatefulWidget {
  ...
  final int gender, id;
  final EMMessage msg;
  final String uheadimg, mheadimg, tk;
  ...
  _avatarWidget() {
    return FlatButton(
      ...
      onPressed: () {
        ...
        // }
        if (1 == widget.gender) {
          if (widget.msg.direction == EMMessageDirection.RECEIVE) {
            // 我的
            Navigator.push(
              context,
              new MaterialPageRoute(
                  builder: (context) => GirlDetailPage(widget.tk, widget.id, widget.gender, widget.mheadimg)),
            );
          } else {
          }
        } else {
          if (widget.msg.direction == EMMessageDirection.RECEIVE) {
            ...
            Navigator.push(
              context,
              new MaterialPageRoute(
                  builder: (context) => GirlDetailPage(widget.tk, _prefs.getInt('account_id'), widget.gender, widget.uheadimg)),
            );
          }
        }
      },
      child: new CircleAvatar(
        backgroundImage: new NetworkImage(widget.msg.direction == EMMessageDirection.RECEIVE ? (null == widget.uheadimg || widget.uheadimg.isEmpty ? '' : widget.uheadimg) : (null == widget.mheadimg || widget.mheadimg.isEmpty ? '' : widget.mheadimg)),
        radius: 18.0),
    );
  }
