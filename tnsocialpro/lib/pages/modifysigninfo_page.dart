...
class _ModifySigninfoPageState extends State<ModifySigninfoPage> {
          ...
          children: [
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                height: 50,
                margin:
                    EdgeInsets.only(bottom: 56, top: 20, left: 42, right: 42),
                alignment: Alignment.center,
                decoration: new BoxDecoration(
                  image: new DecorationImage(
                    image: new AssetImage(
                      ('assets/images/icon_savebg.png'),
    ...
    return signTemples0.map((SignTemple item) {
      return Container(
        ...
        child: AButton.normal(
            ...
            height: 40,
            plain: true,
            bgColor: (null == item.isChoose || !item.isChoose)
                ? rgba(254, 197, 190, 1)
                : rgba(209, 99, 242, 1),
            borderColor: (null == item.isChoose || !item.isChoose)
                ? rgba(254, 197, 190, 1)
                : rgba(209, 99, 242, 1),
            borderRadius: BorderRadius.circular(27.5),
            child: Text(
  ...
  getSigntemplelist() async {
    try {
      var res = await G.req.shop.getSigntemplelist(tk: widget.tk);
      if (res.data != null) {
  ...
  updateUserInfo() async {
    ...
    try {
      String inputsign =
          signdata.toString().substring(1, signdata.toString().length - 1);
      ...
      if (20000 == code) {
        // 修改
        modifySigninfoBus.fire(new ModifySigninfoEvent(
            signdata.toString().substring(1, signdata.toString().length - 1)));
        // setState(() {});
        G.toast('修改成功');
